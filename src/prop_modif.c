...
/**
 * @brief This function will modify the title of the audio file, by deleting str_to_delete if it's present
 * 
...
int modify_title(const char* file_name, const char* str_to_delete, char* new_title) {
    ...
    // we will copy the characters of the title to the new title, except for the ones that are in the string to delete
    for (int i = 0; i < len_file_name; i++) {
        if (!(i >= sub_str - file_name && i < sub_str - file_name + len_str_to_delete)) {
            new_title[i_for_new_title] = file_name[i];
            i_for_new_title++;
        }
    }
    // we will add the null character at the end of the new title and return 0 to say that the function was successful
...
/**
 * @brief This function will modify the path of the audio file or folder, by replacing old_title with new_title
 * 
 * @param path The path of the file or folder
 * @param old_title The old title
 * @param new_title The new title
 * 
 * @return 0 if the function was successful, 4 if the full path could not be created, 6 if the file could not be renamed
*/

int modify_path(const char* path, const char* old_title, const char* new_title) {
    // create the full path for the old and new title
    char* old_full_path = (char*) malloc(PATH_MAX);
    char* new_full_path = (char*) malloc(PATH_MAX);

    int old_full_path_err = snprintf(old_full_path, PATH_MAX, "%s/%s", path, old_title);
    ...
    int new_full_path_err = snprintf(new_full_path, PATH_MAX, "%s/%s", path, new_title);
...
/**
 * @brief This function will modify the title of the file or folder, by deleting str_to_delete if it's present
 * 
 * @param path The path of the file or folder
...
int file_title_modifier(const char* path, const char* title, const char* str_to_delete){
    ...
    if (error_modify_title != 0) {
        ...
        // call the function to modify the path if the title need to be modified and free the memory
...
int prop_modif(const char* folder_path, const char* str_to_find) {
    ...
    while((item = readdir(dir))) {
        ...
        if (item->d_name[0] != '.') {
            ...
            int full_path_err = snprintf(full_path, PATH_MAX, "%s/%s", folder_path, item->d_name);
